...
# Ubuntu Iranian Mirror switcher Script
# Fixed version with comprehensive error handling and security improvements

echo "🔸 Ubuntu Mirror switcher - Replacing default Ubuntu mirrors with Iranian mirrors"
...
backup_sources() {
    ...
    if [ ! -r "/etc/apt/sources.list" ]; then
        echo "❌ Cannot read /etc/apt/sources.list!"
        return 1
    fi
    ...
    if [ -f "$backup_file" ]; then
        ...
        echo "📝 Previous backup exists, creating new backup: $backup_file"
    fi
...
create_sources_list() {
    ...
    if ! validate_url "$mirror_url"; then
        echo "❌ Invalid mirror URL format!"
        return 1
    fi
    ...
    if ! codename=$(lsb_release -sc 2>/dev/null); then
        echo "❌ Failed to get Ubuntu codename!"
        return 1
    fi
    
    # Validate codename
    if [[ -z "$codename" || ! "$codename" =~ ^[a-z]+$ ]]; then
        echo "❌ Invalid Ubuntu codename: '$codename'"
        return 1
    fi
...
# Generated by Ubuntu Mirror switcher
# Mirror: $mirror_url
...
EOF
    ...
    if [ ! -s "$temp_sources" ]; then
        echo "❌ Failed to generate sources.list content!"
        rm -f "$temp_sources"
        return 1
    fi
    ...
    if $SUDO_CMD mv "$temp_sources" /etc/apt/sources.list; then
        ...
        echo "❌ Failed to install new sources.list"
        rm -f "$temp_sources"
        return 1
    fi
}
...
update_packages() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        if timeout 300 $SUDO_CMD apt update 2>/dev/null; then
            ...
            echo "❌ Failed to update package list (attempt $attempt/$max_attempts)"
    ...
    echo "❌ Failed to update package list after $max_attempts attempts"
    return 1
}
...
test_mirror() {
    ...
    if ! curl -s --connect-timeout "$timeout" --max-time "$((timeout * 2))" --head "$test_url" > /dev/null 2>&1; then
        [[ "$quiet" == "false" ]] && echo "❌ Mirror is not accessible (connection failed)"
        return 1
    fi
    ...
    if ! curl -s --connect-timeout "$timeout" --max-time "$((timeout * 2))" "${test_url}/ls-lR.gz" > /dev/null 2>&1 && \
       ! curl -s --connect-timeout "$timeout" --max-time "$((timeout * 2))" "${test_url}/dists/" > /dev/null 2>&1; then
        [[ "$quiet" == "false" ]] && echo "❌ Mirror is accessible but doesn't appear to be an Ubuntu repository"
        return 1
    fi
    ...
    if codename=$(lsb_release -sc 2>/dev/null) && [[ -n "$codename" ]]; then
        if ! curl -s --connect-timeout "$timeout" --max-time "$((timeout * 2))" "${test_url}/dists/${codename}/" > /dev/null 2>&1; then
            [[ "$quiet" == "false" ]] && echo "⚠️  Mirror accessible but may not support Ubuntu $codename"
            # Don't fail completely, just warn
        fi
    fi
    
    [[ "$quiet" == "false" ]] && echo "✅ Mirror is accessible and appears to be a valid Ubuntu repository"
    return 0
}
...
get_custom_mirror() {
    ...
    while [ $attempt -le $max_attempts ]; do
        echo "📝 Enter custom mirror URL (attempt $attempt/$max_attempts):"
        echo "   Example: https://your-mirror.com/ubuntu"
        read -p "URL: " custom_url
        
        # Basic input validation
        if [[ -z "$custom_url" ]]; then
            echo "❌ Empty URL provided!"
            ((attempt++))
            continue
        fi
        ...
        if ! validate_url "$custom_url"; then
            echo "❌ Invalid URL format! Please enter a valid HTTP/HTTPS URL."
        ...
        echo "🔍 Testing custom mirror..."
        if test_mirror "$custom_url" "false" 20; then
            selected_mirror="$custom_url"
            echo "✅ Custom mirror validated successfully!"
            return 0
        else
            echo "⚠️  Custom mirror is not accessible or invalid."
            ...
            case "$choice" in
                "1")
                    ...
                    ;;
                "2")
                    echo "⚠️  Using potentially problematic mirror..."
                    selected_mirror="$custom_url"
                    return 0
                    ;;
                "3")
                    ...
                    ;;
                *)
                    echo "❌ Invalid option! Trying again..."
                    ((attempt++))
                    ;;
            esac
        fi
    ...
    echo "❌ Maximum attempts reached. Returning to mirror selection."
    return 1
}
        ...
        case "$selection" in
            ...
            *)
                ...
                if [[ "$selection" =~ ^[0-9]+$ ]] && [[ "$selection" -ge 1 && "$selection" -le "${#MIRRORS[@]}" ]]; then
                    ...
                    echo "❌ Invalid selection! Please enter a number between 0 and $((${#MIRRORS[@]}+2))."
                    echo
                fi
                ;;
        esac
...
check_requirements() {
    ...
    if [ ! -w "$(dirname /etc/apt/sources.list)" ]; then
        echo "❌ Cannot write to /etc/apt/ directory. Please run with appropriate permissions."
        return 1
    fi
    ...
    if ! arch=$(dpkg --print-architecture 2>/dev/null); then
        echo "⚠️  Warning: Could not determine system architecture."
    else
    ...
    for cmd in "${missing_commands[@]}"; do
        if ! command -v "$cmd" &> /dev/null; then
            echo "❌ Required command '$cmd' is still not available after dependency installation!"
            return 1
        fi
    done
    ...
    if version_id=$(lsb_release -rs 2>/dev/null); then
        ...
        case "$version_id" in
            "14.04"|"16.04"|"17."*|"18.10"|"19."*|"20.10"|"21."*)
                echo "⚠️  Warning: You're using an older Ubuntu version ($version_id)."
...
main() {
    echo "Starting Ubuntu Mirror switcher..."
    echo
    ...
    # Update package list
    echo "🔄 Testing the new mirror configuration..."
    if ! update_packages; then
        echo "❌ Failed to update with selected mirror. Restoring backup..."
        restore_backup
        if update_packages; then
            echo "✅ Original configuration restored and working"
        fi
        echo "⚠️  Mirror change failed. The selected mirror may not be compatible with your Ubuntu version."
        exit 1
    fi
    ...
    if [ $exit_code -ne 0 ]; then
        ...
        if [ -n "$BACKUP_FILE" ] && [ -f "$BACKUP_FILE" ]; then
            ...
            if restore_backup; then
                ...
                echo "❌ Failed to restore backup! Manual intervention may be required."
